...
- *Filtering* - Filter by name regexp or file size range
...
- *Enhanced header line* - Shows current image info, directory location, dimensions, sort/filter status
...
| Key     | Command                                   | Description                              |
|---------+-------------------------------------------+------------------------------------------|
| =n=, =f=, =→= | ~dired-image-thumbnail-next-image~          | Next image (auto-display when enabled)   |
| =p=, =b=, =←= | ~dired-image-thumbnail-previous-image~      | Previous image (auto-display when enabled) |
| =+=     | ~dired-image-thumbnail-increase-size~       | Increase thumbnail size                  |
| =-=     | ~dired-image-thumbnail-decrease-size~       | Decrease thumbnail size                  |

Note: When =dired-image-thumbnail-auto-display-on-navigate= is enabled (default), pressing =n= or =p= (also =f= or =b=) automatically displays the full-size image in the right window. When follow is activated and the image window opens, the thumbnails are automatically refreshed so they re-align and the columns fit the new thumbnail window width.

*** Marking

| Key   | Command                                    | Description                     |
|-------+--------------------------------------------+---------------------------------|
| =s=, =S= | ~dired-image-thumbnail-sort~                 | Sort via completing-read        |

*** Filtering

| Key   | Command                                    | Description                |
|-------+--------------------------------------------+----------------------------|
| =/=, =\= | ~dired-image-thumbnail-filter~               | Filter via completing-read |

*** Display

| Key       | Command                                      | Description                    |
|-----------+----------------------------------------------+--------------------------------|
| =g=       | ~dired-image-thumbnail-refresh~                | Refresh the display            |
| =G=       | ~dired-image-thumbnail-hard-refresh~           | Clear cache and refresh        |
| =+=       | ~dired-image-thumbnail-increase-size~          | Increase thumbnail size        |
| =-=       | ~dired-image-thumbnail-decrease-size~          | Decrease thumbnail size        |
| =Q=       | ~dired-image-thumbnail-select-display-quality~  | Select quality via completing-read |

*** Marking

| Key | Command                                 | Description                                |
|-----+-----------------------------------------+--------------------------------------------|
| =m= | ~dired-image-thumbnail-mark~              | Mark image and follow to the next          |
| =u= | ~dired-image-thumbnail-unmark~            | Unmark image and follow to the next        |
...
| =?=   | ~dired-image-thumbnail-transient~        | Transient menu (all commands)  |
...
- *Image count* - Number of images, shown at the start (e.g., =[156 images]=); with a filter on, the filtered set over the total (e.g., =[12/156 images]=)
- *Marked count* - Number of marked images, shown when one or more are marked (e.g., =[3 marked]=)
...
#+begin_src emacs-lisp
(setq dired-image-thumbnail-sort-by 'dired)      ; 'dired, 'name, 'date, or 'size
...
#+begin_src emacs-lisp
(setq dired-image-thumbnail-display-quality 'faster) ; 1 full, 2 high, 3 fast, 4 faster, or 5 draft
#+end_src

The current quality and thumbnail size are shown in the thumbnail buffer
mode line right after the buffer name, e.g. =[Q4 faster] [160px]=.  The
size segment follows the =+= / =-= zoom keys.
...
The display quality (=Q=), sort criteria/order (=s=), name/size filters (=/=)
and thumbnail size (=+= / =-=) are remembered per directory in that directory's
=.dir-locals.el=.  Revisiting a directory restores its choices, so folders with
typically small images (e.g. screenshots) can keep =high= quality while large
photo collections use a faster mode:

#+begin_src emacs-lisp
;; ~/.DCIM/Screenshots/.dir-locals.el (written automatically)
((nil . ((dired-image-thumbnail-display-quality . high)
         (dired-image-thumbnail-default-display-size . 192))))
#+end_src

Directories without a remembered size keep the current zoom, unless
=dired-image-thumbnail-default-display-size= is set, in which case it provides
the default for them.

Saving is automatic and merges with any existing entries.  Use =C-u Q= for a
session-only quality change, or disable the automatic saving entirely:

#+begin_src emacs-lisp
(setq dired-image-thumbnail-save-dir-settings nil) ; keep choices session-local
#+end_src
...
- From the thumbnail buffer, =z= (or the transient menu's Other group) inserts subdirectories recursively into the associated dired buffer; the =i= / =I= / =K= variants remain available from dired via =C-t= bindings
