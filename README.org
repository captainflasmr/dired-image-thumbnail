...
The current quality is shown in the thumbnail buffer mode line right after the buffer name, e.g. =[Q4 faster]=.
#+end_src
...
The display quality (=Q=), sort criteria/order (=s=) and name/size filters (=/=)
are remembered per directory in that directory's =.dir-locals.el=.  Revisiting
a directory restores its choices, so folders with typically small images (e.g.
screenshots) can keep =high= quality while large photo collections use a faster
mode:
