...
- *Marked count* - Number of marked images, always shown (e.g., =[3 marked]=)
- *Filter count* - When a filter is active, the number of matching images over the total (e.g., =[12/156]=)
...
#+begin_src emacs-lisp
(setq dired-image-thumbnail-display-quality 'faster) ; 'full, 'high, 'fast, 'faster, or 'draft
#+end_src
